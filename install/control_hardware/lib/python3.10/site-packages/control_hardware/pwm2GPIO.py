...
class PwmSubNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        if self.is_reverse_l.value:
            self.pwm_l.value = min( buff *abs(msg.pwm_l/1000) ,0.52)#max(0.0, min(msg.pwm_l / 100.0, 1.0))  # Scale from 0-100 to 0-1
        else:
            self.pwm_l.value = min( buff * abs(msg.pwm_l/1000) ,0.52)

        if self.is_reverse_r.value:
            self.pwm_r.value = min( buff * abs(msg.pwm_r/1000) ,0.52)#max(0.0, min(msg.pwm_l / 100.0, 1.0))  # Scale from 0-100 to 0-1
        else:
            self.pwm_r.value = min( buff * abs(msg.pwm_r/1000) ,0.52)
